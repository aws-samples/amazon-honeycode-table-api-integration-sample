...
const environment = require('../lambda/ImportCustomersDynamoDB/env.json');
...
function importCustomersDynamoDB(scope) {
    ...
    importCustomersLambda.role.addToPrincipalPolicy(workbookPolicy)
    const tablePolicy = new iam.PolicyStatement();
    tablePolicy.addActions(['honeycode:ListTableColumns', 'honeycode:BatchCreateTableRows', 'honeycode:BatchUpdateTableRows', 'honeycode:BatchDeleteTableRows', 'honeycode:QueryTableRows']);
    tablePolicy.addResources([`arn:aws:honeycode:*:*:table:workbook/${environment.workbookId}/table/*`]);
    importCustomersLambda.role.addToPrincipalPolicy(tablePolicy)
